...
if __name__ == '__main__':
    ...
    print(f"Loading from {agg_m}")
    model.agg_pred.load_state_dict(torch.load(agg_m))
    print(f"Loading from {sel_m}")
    model.selcond_pred.load_state_dict(torch.load(sel_m))
    print(f"Loading from {cond_m}")
    model.op_str_pred.load_state_dict(torch.load(cond_m))
    #only for loading trainable embedding
    print(f"Loading from {agg_e}")
    model.agg_type_embed_layer.load_state_dict(torch.load(agg_e))
    print(f"Loading from {sel_e}")
    model.sel_type_embed_layer.load_state_dict(torch.load(sel_e))
    print(f"Loading from {cond_e}")
    model.cond_type_embed_layer.load_state_dict(torch.load(cond_e))


    acc_qm, one_acc = epoch_acc(model, BATCH_SIZE, val_sql_data, val_table_data, TEST_ENTRY, args.db_content)
    print(f"Dev acc_qm: {acc_qm};\n breakdown on (agg, sel, where): {one_acc}")
    epoch_acc = epoch_exec_acc(model, BATCH_SIZE, val_sql_data, val_table_data, DEV_DB, args.db_content)
    print(f"Dev execution acc: {epoch_acc}")
    test_acc, one_test_acc = epoch_acc(model, BATCH_SIZE, test_sql_data, test_table_data, TEST_ENTRY, args.db_content)
    print(f"Test acc_qm: {test_acc};\n breakdown on (agg, sel, where): {one_test_acc}")
    epoch_test_acc = epoch_exec_acc(model, BATCH_SIZE, test_sql_data, test_table_data, TEST_DB, args.db_content)
    print(f"Test execution acc: {epoch_test_acc}")
